...
@section('content')
@php
    $birthValue = old('birth_date', $employee->birth_date);
    if ($birthValue) {
        $birthValue = substr((string) $birthValue, 0, 10);
    }
    $photoUrl = $employee->formal_picture ? asset($employee->formal_picture) : null;
@endphp
...
<div class="data-page employee-form-page mt-2">
    <header class="student-edit-header">
        <div>
            ...
            <h1 class="student-edit-title">{{ $employee->lastname }}, {{ $employee->firstname }}</h1>
            <p class="student-edit-subtitle">
                Employee ID <strong>{{ $employee->employee_id ?? '—' }}</strong>
                @if($employee->department || $employee->position)
                    &middot; {{ trim(($employee->department ?? '').' · '.($employee->position ?? '')) }}
                @endif
            </p>
        </div>
    </header>
                ...
                <div class="student-edit-panel student-edit-identity">
                    ...
                    <div class="student-edit-panel__body text-center">
                        <div class="student-edit-avatar-wrap">
                            @if($photoUrl)
                                <img src="{{ $photoUrl }}" alt="" class="student-edit-avatar" id="profilePreview">
                            @else
                        ...
                        <div class="student-edit-field text-start">
                            ...
                            <input type="text" id="qrcode" class="form-control form-control-sm bg-light font-monospace" value="{{ $employee->qrcode ?? '—' }}" readonly>
                        ...
                        <div class="student-edit-field text-start">
                            ...
                            <input type="text" name="rfid" id="rfid" class="form-control form-control-sm @error('rfid') is-invalid @enderror"
                                   value="{{ old('rfid', $employee->rfid) }}" placeholder="Scan or type tag"
                                   autocomplete="off">
                    ...
                    <div class="student-edit-panel__body">
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="date" name="birth_date" id="birth_date" class="form-control" value="{{ $birthValue }}">
                            </div>
                            <div class="col-md-4">
                                <label for="sex" class="form-label">Sex</label>
                                <select name="sex" id="sex" class="form-select">
                                    <option value="">Select…</option>
                                    <option value="Male" @selected(old('sex', $employee->sex) === 'Male')>Male</option>
                                    <option value="Female" @selected(old('sex', $employee->sex) === 'Female')>Female</option>
                                    <option value="Other" @selected(old('sex', $employee->sex) === 'Other')>Other</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="tin_id_number" id="tin_id_number" class="form-control"
                                       value="{{ old('tin_id_number', $employee->tin_id_number) }}">
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="hdmf_number" id="hdmf_number" class="form-control"
                                       value="{{ old('hdmf_number', $employee->hdmf_number) }}">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                @if($employee->employee_signature)
                                    ...
                                    <img src="{{ asset($employee->employee_signature) }}" alt="Current signature" class="student-edit-sig-preview">
                                @endif
                                <div class="signature-wrap">
                                    <canvas id="employeeSignaturePad"></canvas>
                                </div>
                                <input type="hidden" name="employee_signature" id="employeeSignatureInput">
                                <button type="button" id="clearEmployeeSignature" class="btn btn-sm btn-outline-secondary mt-2">Clear new signature</button>
                            </div>
                        </div>
                    </div>
                </div>
